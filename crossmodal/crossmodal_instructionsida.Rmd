...
```{r}
pacman::p_load(tidyverse, stringr, readbulk, patchwork, lmerTest)
```
...
Load in the data.
```{r}
files <- list.files(path = "data_aud", pattern = ".csv", full.names = T)


dataaud <- map_dfr(files, read.csv)


files2 <- list.files(path = "data_vis", pattern = ".csv", full.names = T)


datavis <- map_dfr(files2, read.csv)
```
